...
int main(void)
{
	...
	int strLen;
	int shmid,shmid2,shmid3, shmid4; //shared memory
...
//	}
	...
	//공유메모리 공간 만들기 3
	shmid4 = shmget((key_t)skey4, sizeof(int), 0777|IPC_CREAT);
	if(shmid4 == -1)
	{
		perror("shmget failed : ");
		exit(0);
	...
	//공유메모리 맵핑 3
	shared_memory4 = shmat(shmid4, (void *)0, 0);
	if(!shared_memory4)
	{
		perror("shmat failed");
		exit(0);
	...
	//공유메모리 변수 생성
	  ...
	  int* w_brake;
	  w_brake = (int *)shared_memory2;
	  int* w_accel;
	  w_accel = (int *)shared_memory3;
	  int* w_brake2;
	  w_brake2 = (int *)shared_memory4;
		...
		//   str2 += " ";
			 ...
			 if(count == 10)
			 {
				*w_steer = atoi(ptr);
				...
				if(*w_steer == 0) {
					*w_steer = atoi(to_string.substr(6, 4).c_str());
				}
				printf("steer : %d\n", *w_steer);
				break;
			 }
