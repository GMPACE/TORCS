/***************************************************************************
    ...
    version              : $Id: berniw3.cpp,v 1.2.2.4 2013/08/05 17:22:43 berniw Exp $
...
/* Module entry point */
extern "C" int berniw3(tModInfo *modInfo)
{
	//char	buffer[BUFSIZE];
	...
	*dist_to_ocar = 10000000.0;
	...
	if (currenttime != situation->currentTime) {
		...
		for (int i = 0; i < situation->_ncars; i++) {
			...
			if(ocar[i].getCarPtr()->pub.trkPos.toLeft < ocar[i].getCarPtr()->pub.trkPos.toRight)
