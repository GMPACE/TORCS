...
static void AbandonRaceHookActivate(void * /* vforce */) {
...
static void AbortRaceHookActivate(void * /* dummy */) {
	GfuiScreenActivate(ReInfo->_reGameScreen);
...
void ReInitRules(tRmInfo* ReInfo) {
	...
	const char* value = GfParmGetStr(ReInfo->params, ReInfo->_reRaceName,
	RM_ATTR_INVALIDATE_BEST_LAP_WALL_TOUCH, RM_VAL_YES);
	...
	value = GfParmGetStr(ReInfo->params, ReInfo->_reRaceName,
	RM_ATTR_INVALIDATE_BEST_LAP_CORNER_CUT, RM_VAL_YES);
	if (!strcmp(value, RM_VAL_YES)) {
		ReInfo->raceRules.enabled |=
				RmRaceRules::CORNER_CUTTING_TIME_INVALIDATE;
	}

	// Time penalty for corner cutting?
	value = GfParmGetStr(ReInfo->params, ReInfo->_reRaceName,
	RM_ATTR_CORNER_CUT_TIME_PENALTY, RM_VAL_YES);
	if (!strcmp(value, RM_VAL_YES)) {
		ReInfo->raceRules.enabled |= RmRaceRules::CORNER_CUTTING_TIME_PENALTY;
	}

	// Fuel consumption factor
	tdble number = GfParmGetNum(ReInfo->params, ReInfo->_reRaceName,
	RM_ATTR_FUEL_FACTOR, NULL, 1.0f);
	if (number < 0.0f)
		number = 0.0f;	// Avoid negative factor
	ReInfo->raceRules.fuelFactor = number;

	// Damage factor
	number = GfParmGetNum(ReInfo->params, ReInfo->_reRaceName,
	RM_ATTR_DAMAGE_FACTOR, NULL, 1.0f);
	if (number < 0.0f)
		number = 0.0f;	// Avoid negative factor
	...
	// Refuel fuel flow
	number = GfParmGetNum(ReInfo->params, ReInfo->_reRaceName,
	RM_ATTR_REFUEL_FUEL_FLOW, NULL, 8.0f);
	if (number < 1.0f)
		number = 1.0f;	// Avoid division by zero or negative pit times
	ReInfo->raceRules.refuelFuelFlow = number;

	// Damage repair factor
	number = GfParmGetNum(ReInfo->params, ReInfo->_reRaceName,
	RM_ATTR_DAMAGE_REPAIR_FACTOR, NULL, 0.007f);
	if (number < 0.0f)
		number = 0.0f;	// Avoid negative pit times
	ReInfo->raceRules.damageRepairFactor = number;

	// Pit stop base time (time for a stop even if nothing is done)
	number = GfParmGetNum(ReInfo->params, ReInfo->_reRaceName,
	RM_ATTR_PITSTOP_BASE_TIME, NULL, 2.0f);
	if (number < 0.0f)
		number = 0.0f;	// Avoid negative pit times
	...
	number = GfParmGetNum(ReInfo->params, ReInfo->_reRaceName,
	RM_ATTR_PIT_SPEED_LIMIT, NULL, number);
...
int RePreRace(void) {
	...
	if (dist < 0.001) {
		ReInfo->s->_totLaps = (int) GfParmGetNum(params, raceName, RM_ATTR_LAPS,
		NULL, 30);
	} else {
		ReInfo->s->_totLaps = ((int) (dist / ReInfo->track->length)) + 1;
	}
	ReInfo->s->_maxDammage = (int) GfParmGetNum(params, raceName,
	RM_ATTR_MAX_DMG, NULL, 10000);

	const char* raceType = GfParmGetStr(params, raceName, RM_ATTR_TYPE,
	RM_VAL_RACE);
	...
	/* Cleanup results */
	snprintf(path, BUFSIZE, "%s/%s/%s", ReInfo->track->name, RE_SECT_RESULTS,
			raceName);
...
static int reRaceRealStart(void) {
	...
	snprintf(buf, BUFSIZE, "%smodules/simu/%s.%s", GetLibDir(), dllname,
	DLLEXT);
	if (GfModLoad(0, buf, &ReRaceModList))
		return RM_QUIT;
	ReRaceModList->modInfo->fctInit(ReRaceModList->modInfo->index,
			&ReInfo->_reSimItf);
	...
	if (!(ReInfo->s->_raceType == RM_TYPE_QUALIF)
			|| ((int) GfParmGetNum(results, RE_SECT_CURRENT, RE_ATTR_CUR_DRIVER,
			NULL, 1) == 1)) {
		RmLoadingScreenStart(ReInfo->_reName, "data/img/splash-qrloading.png");
	}
	...
	if (ReInfo->_displayMode == RM_DISP_MODE_NONE) {
		if (ReInfo->s->_raceType == RM_TYPE_QUALIF) {
			...
			snprintf(buf, BUFSIZE, "%s on %s", s->cars[0]->_name,
					ReInfo->track->name);
	...
	if ((ReInfo->_displayMode != RM_DISP_MODE_CONSOLE)
			&& ReInfo->_reGraphicItf.initview != 0) {
		GfScrGetSize(&sw, &sh, &vw, &vh);
		ReInfo->_reGraphicItf.initview((sw - vw) / 2, (sh - vh) / 2, vw, vh,
		GR_VIEW_STD, ReInfo->_reGameScreen);
...
void init_shared_memory() {
	...
	shmid_ui = shmget((key_t) skey_ui, sizeof(int), 0777 | IPC_CREAT);
	if (shmid_ui == -1) {
		perror("shmget failed");
		exit(0);
	}
	shared_memory_ui[0] = (char*) shmat(shmid_ui, (void *) 0, 0);
	if (!shared_memory_ui[0]) {
		perror("shmat failed");
		exit(0);
	}
	send_data_ui[0] = shared_memory_ui[0];
}
...
int ReRaceStart(void) {
	...
	FREEZ(ReInfo->_reCarInfo);
	ReInfo->_reCarInfo = (tReCarInfo*) calloc(
			GfParmGetEltNb(params, RM_SECT_DRIVERS), sizeof(tReCarInfo));
	...
	if (ReInfo->s->_raceType == RM_TYPE_QUALIF) {
		i = (int) GfParmGetNum(results, RE_SECT_CURRENT, RE_ATTR_CUR_DRIVER,
		NULL, 1);
		if (i == 1) {
			RmLoadingScreenStart(ReInfo->_reName,
					"data/img/splash-qrloading.png");
		...
		const char* gridType = GfParmGetStr(params, raceName,
		...
		if (!strcmp(gridType, RM_VAL_LAST_RACE_ORDER)) {
			...
			maxCars = (int) GfParmGetNum(params, raceName, RM_ATTR_MAX_DRV,
			NULL, 100);
			...
			for (i = 1; i < nCars + 1; i++) {
				snprintf(path, BUFSIZE, "%s/%s/%s/%s/%d", ReInfo->track->name,
				RE_SECT_RESULTS, prevRaceName, RE_SECT_RANK, i);
			...
			for (i = 1; i < nCars + 1; i++) {
				snprintf(path, BUFSIZE, "%s/%s/%s/%s/%d", ReInfo->track->name,
				RE_SECT_RESULTS, prevRaceName, RE_SECT_RANK, nCars - i + 1);
				snprintf(path2, BUFSIZE, "%s/%d", RM_SECT_DRIVERS_RACING, i);
				GfParmSetStr(params, path2, RM_ATTR_MODULE,
						GfParmGetStr(results, path, RE_ATTR_MODULE, ""));
				GfParmSetNum(params, path2, RM_ATTR_IDX, NULL,
	...
	if (ReInfo->_displayMode != RM_DISP_MODE_CONSOLE) {
		if (!strcmp(
				GfParmGetStr(params, ReInfo->_reRaceName, RM_ATTR_SPLASH_MENU,
				RM_VAL_NO), RM_VAL_YES)) {
			RmShutdownLoadingScreen();
			RmDisplayStartRace(ReInfo, StartRaceHookInit(),
					AbandonRaceHookInit());
			return RM_ASYNC | RM_NEXT_STEP;
		}
	}
...
int ReRaceStop(void) {
	...
	if (!strcmp(GfParmGetStr(params, ReInfo->_reRaceName, RM_ATTR_ALLOW_RESTART,
	RM_VAL_NO), RM_VAL_NO)) {
		StopScrHandle = RmTriStateScreen("Race Stopped", "Abandon Race",
				"Abort current race", AbortRaceHookInit(), "Resume Race",
				"Return to Race", BackToRaceHookInit(), "Quit Game",
				"Quit the game", QuitHookInit());
	} else {
		if ((ReInfo->s->raceInfo.type == RM_TYPE_PRACTICE
				|| ReInfo->s->raceInfo.type == RM_TYPE_QUALIF)
				&& (ReInfo->s->raceInfo.ncars == 1)
				&& (ReInfo->carList[0].info.driverType == RM_DRV_HUMAN)) {
			tCarElt* carElt = &ReInfo->carList[0];
			static const char* label[5] = { "Restart Race",
					"Setup Car, Restart", "Abandon Race", "Resume Race",
					"Quit Game" };
			static const char* tip[5] =
					{ "Restart the current race",
							"Setup car and restart the current race",
							"Abort the current race", "Return to the race",
							"Quit TORCS" };
			void* screen[5];

			screen[0] = RestartRaceHookInit();
			screen[1] = RmCarSetupScreenInit(RestartRaceHookInit(), carElt,
					ReInfo);
			screen[2] = AbortRaceHookInit();
			screen[3] = BackToRaceHookInit();
			screen[4] = QuitHookInit();

			StopScrHandle = RmNStateScreen("Race Stopped", label, tip, screen,
					5);
		} else {
			StopScrHandle = RmFourStateScreen("Race Stopped", "Restart Race",
					"Restart the current race", RestartRaceHookInit(),
...
int ReRaceEnd(void) {
	...
	if (ReInfo->s->_raceType == RM_TYPE_QUALIF) {
		curDrvIdx = (int) GfParmGetNum(results, RE_SECT_CURRENT,
...
int RePostRace(void) {
	...
	curRaceIdx = (int) GfParmGetNum(results, RE_SECT_CURRENT, RE_ATTR_CUR_RACE,
	NULL, 1);
	if (curRaceIdx < GfParmGetEltNb(params, RM_SECT_RACES)) {
		curRaceIdx++;
		GfOut("Race Nb %d\n", curRaceIdx);
		GfParmSetNum(results, RE_SECT_CURRENT, RE_ATTR_CUR_RACE, NULL,
				curRaceIdx);
		ReUpdateStandings();
		return RM_SYNC | RM_NEXT_RACE;
	}
...
int ReEventShutdown(void) {
	...
	RE_ATTR_CUR_RACE, NULL, 1);
	curTrkIdx = (int) GfParmGetNum(results, RE_SECT_CURRENT, RE_ATTR_CUR_TRACK,
	NULL, 1);
