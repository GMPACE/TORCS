...
static void ReUpdtPitTime(tCarElt *car) {
	...
	case RM_PIT_REPAIR:
		info->totalPitTime =
				ReInfo->raceRules.pitstopBaseTime
						+ fabs((double) (car->_pitFuel))
								/ ReInfo->raceRules.refuelFuelFlow+ (tdble)(fabs((double)(car->_pitRepair))) * ReInfo->raceRules.damageRepairFactor + car->_penaltyTime;
...
static void ReManage(tCarElt *car) {
	...
	for (int i = 0; i < ReInfo->s->raceInfo.ncars; i++) {
		data.append(ReInfo->carList[i].info.name).append(":").append(
				to_string(ReInfo->carList[i].pub.DynGC.pos.x)).append(",").append(
				to_string(ReInfo->carList[i].pub.DynGC.pos.y)).append("%").append(
				to_string(ReInfo->carList[i].pub.trkPos.toLeft)).append("@").append(
				to_string(ReInfo->carList[i].race.distRaced)).append("$").append(
				to_string(ReInfo->carList[i].pub.driver_intent)).append("&");
	}
	//printf("%s\n",data.c_str());
	...
	if (car->_pit) {
		...
		if (car->_state & RM_CAR_STATE_PIT) {
			...
			if (car->_scheduledEventTime < s->currentTime) {
				...
				snprintf(buf, BUFSIZE, "%s pit stop %.1fs", car->_name,
						info->totalPitTime);
				ReRaceMsgSet(buf, 5);
			} else {
				snprintf(car->ctrl.msg[2], 32, "in pits %.1fs",
			...
			if ((lgFromStart > car->_pit->lmin)
					&& (lgFromStart < car->_pit->lmax)) {
				pitok = 0;
				int side;
				tdble toBorder;
				...
				if (pitok) {
					...
					ReRaceMsgSet(buf, 5);
					if (car->robot->rbPitCmd(car->robot->index, car,
							s) == ROB_PIT_MENU) {
				...
				if ((car->_state & RM_CAR_STATE_FINISH) == 0) {
					...
					if (car->_laps > 1) {
						...
						if (car->_bestLapTime != 0) {
							car->_deltaBestLapTime = car->_lastLapTime
									- car->_bestLapTime;
						}
						if ((car->_lastLapTime < car->_bestLapTime)
								|| (car->_bestLapTime == 0)) {
						...
						if (car->_pos != 1) {
							car->_timeBehindLeader = car->_curTime
									- s->cars[0]->_curTime;
							car->_lapsBehindLeader = s->cars[0]->_laps
									- car->_laps;
							car->_timeBehindPrev = car->_curTime
									- s->cars[car->_pos - 2]->_curTime;
							s->cars[car->_pos - 2]->_timeBeforeNext =
									car->_timeBehindPrev;
						} else {
							...
							if (ReInfo->_displayMode == RM_DISP_MODE_CONSOLE) {
								printf(
										...
										s->currentTime, car->_laps - 1,
										car->_distRaced / 1000.0f);
							}
						}
						...
						case RM_TYPE_PRACTICE:
							if (ReInfo->_displayMode == RM_DISP_MODE_NONE) {
								...
								GfTime2Str(t1, TIMEFMTSIZE, car->_lastLapTime,
										0);
								GfTime2Str(t2, TIMEFMTSIZE, car->_bestLapTime,
										0);
								snprintf(buf, BUFSIZE,
										"lap: %02d   time: %s  best: %s  top spd: %.2f    min spd: %.2f    damage: %d",
										car->_laps - 1, t1, t2,
										info->topSpd * 3.6, info->botSpd * 3.6,
										car->_dammage);
					...
					if ((car->_remainingLaps < 0)
						...
						if (ReInfo->s->_raceType == RM_TYPE_RACE) {
							if (car->_pos == 1) {
								...
								snprintf(buf, BUFSIZE, "%s Finished %d%s",
	...
	car->_distFromStartLine = car->_trkPos.seg->lgfromstart
			+ (car->_trkPos.seg->type == TR_STR ?
					car->_trkPos.toStart :
					car->_trkPos.toStart * car->_trkPos.seg->radius);
	car->_distRaced = (car->_laps - (info->lapFlag + 1))
			* ReInfo->track->length+ car->_distFromStartLine;
}
...
static void ReRaceRules(tCarElt *car) {
	...
	if ((car->pub.state & RM_CAR_STATE_FINISH) == 0) {
		...
		if (ReInfo->raceRules.enabled & RmRaceRules::WALL_HIT_TIME_INVALIDATE) {
			if (car->_commitBestLapTime
					&& (car->priv.simcollision & SEM_COLLISION_XYSCENE)) {
		...
		if (mainseg->type != TR_STR) {
			if (track->pits.type == TR_PIT_ON_TRACK_SIDE) {
				if (pitInfo.pitEntry->id < pitInfo.pitExit->id) {
					if ((mainseg->id >= pitInfo.pitEntry->id)
							&& (mainseg->id <= pitInfo.pitExit->id)) {
						pit = true;
					}
				} else {
					if ((mainseg->id >= pitInfo.pitEntry->id)
		...
		if (toborder < -cuttinglimit) {
			if (ReInfo->raceRules.enabled
					& RmRaceRules::CORNER_CUTTING_TIME_INVALIDATE) {
				if (ReInfo->s->_raceType != RM_TYPE_RACE
						&& car->_commitBestLapTime) {
			...
			if (ReInfo->s->_raceType == RM_TYPE_RACE
				...
				if (minradius > 1.0f) {
					car->_penaltyTime += car->pub.speed * RCM_MAX_DT_SIMU
							* (-toborder - cuttinglimit) / minradius;
				}
			}
		}
	}
	...
	if (prevSeg->raceInfo & TR_PITSTART) {
		...
		if (seg->raceInfo & TR_PIT) {
			...
			if (penalty) {
				switch (penalty->penalty) {
				case RM_PENALTY_DRIVETHROUGH:
					snprintf(buf, BUFSIZE, "%s DRIVE THROUGH PENALTY CLEANING",
							car->_name);
					ReRaceMsgSet(buf, 5);
					rules->ruleState |= RM_PNST_DRIVETHROUGH;
					break;
				case RM_PENALTY_STOPANDGO:
					snprintf(buf, BUFSIZE, "%s STOP&GO PENALTY CLEANING",
							car->_name);
					ReRaceMsgSet(buf, 5);
		...
		if (seg->raceInfo & TR_PIT) {
			...
			if (car->_state & RM_CAR_STATE_PIT) {
				...
				if (rules->ruleState & RM_PNST_STOPANDGO_OK
			...
			if (rules->ruleState
					& (RM_PNST_DRIVETHROUGH | RM_PNST_STOPANDGO_OK)) {
				/* clear the penalty */
	...
	if (seg->raceInfo & TR_SPEEDLIMIT) {
		if (!(rules->ruleState & (RM_PNST_SPD | RM_PNST_STNGO))
...
static void reCapture(void) {
	...
	glReadPixels((sw - vw) / 2, (sh - vh) / 2, vw, vh, GL_RGB, GL_UNSIGNED_BYTE,
			(GLvoid*) img_arr[index]);
	char* msg = "Capture";
	if (index) {
		unsigned char* img_result = (unsigned char*) malloc(vw * vh * 3 * 2 / 2);
		memcpy(img_result, img_arr[0] + vw * vh  * 3 / 4, vw * vh * 3 / 4);
		memcpy(img_result + vw * vh * 3 / 4, img_arr[1] + vw * vh * 3 / 4, vw * vh * 3 / 4);
		//snprintf(buf, BUFSIZE, "%s/torcs-%4.4d-%8.8d.png", capture->outputBase,	capture->currentCapture, capture->currentFrame++);
		snprintf(buf, BUFSIZE, "%s/%d-%d.png",capture->outputBase,capture->currentFrame++, ldws_value);
...
int ReUpdate(void) {
	...
	case RM_DISP_MODE_NORMAL:
		...
		while ((ReInfo->_reRunning
				&& ((t - ReInfo->_reCurTime) > RCM_MAX_DT_SIMU))
				&& MAXSTEPS > i++) {
...
void turn_signal_l(void *num) {
	char* msg;
	if (!strcmp(mycar->info.name, "Player")) {
		if (mycar->pub.driver_intent == 1) {
			mycar->pub.driver_intent = 0;
			msg = "no lc intent";
		} else {
			mycar->pub.driver_intent = 1;
			msg = "left on";
		}
		ReRaceBigMsgSet(msg, 1.5);
	}
...
void turn_signal_r(void *num) {
	char* msg;
	if (!strcmp(mycar->info.name, "Player")) {
		if (mycar->pub.driver_intent == 2) {
			mycar->pub.driver_intent = 0;
			msg = "no lc intent";
		} else {
			mycar->pub.driver_intent = 2;
			msg = "right on";
		}
		ReRaceBigMsgSet(msg, 1.5);
	}
