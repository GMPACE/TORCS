...
struct within {
	...
	GF_TAILQ_ENTRY (struct within)
	linkWithin; /**< Link to next entry */
};
...
struct param {
	...
	GF_TAILQ_ENTRY (struct param)
	linkParam; /**< Next parameter in the same section */
};
...
struct section {
	...
	GF_TAILQ_ENTRY (struct section)
	linkSection; /**< Next section at the same level */
...
struct parmHandle {
	...
	GF_TAILQ_ENTRY (struct parmHandle)
	linkHandle; /**< Next configuration handle */
};
...
static struct param *getParamByName(struct parmHeader *conf,
		const char *sectionName, const char *paramName, int flag);
static void removeParamByName(struct parmHeader *conf, const char *sectionName,
		const char *paramName);
static void removeParam(struct parmHeader *conf, struct section *section,
		struct param *param);
static struct param *addParam(struct parmHeader *conf, struct section *section,
...
void GfParmShutdown(void) {
	...
	while ((parmHandle = GF_TAILQ_FIRST(&parmHandleList))
			!= GF_TAILQ_END(&parmHandleList)) {
...
static void removeParam(struct parmHeader *conf, struct section *section,
	...
	while ((within = GF_TAILQ_FIRST(&param->withinList))
			!= GF_TAILQ_END(&param->withinList)) {
...
static void removeSection(struct parmHeader *conf, struct section *section) {
	...
	if (section->fullName) {
		...
		GF_TAILQ_REMOVE(&(section->parent->subSectionList), section,
				linkSection);
		while ((param = GF_TAILQ_FIRST(&(section->paramList)))
				!= GF_TAILQ_END(&(section->paramList))) {
			removeParam(conf, section, param);
		}
...
static struct parmHeader *getSharedHeader(const char *file, int mode) {
	...
	if ((mode & GFPARM_RMODE_PRIVATE) == 0) {
		for (parmHandle = GF_TAILQ_FIRST(&parmHandleList);
				parmHandle != GF_TAILQ_END(&parmHandleList); parmHandle =
						GF_TAILQ_NEXT(parmHandle, linkHandle)) {
 ...
 */
static void xmlStartElement(void *userData, const char *name,
		const char **atts) {
	...
	if (!strcmp(name, "params")) {
		...
		if (strlen(parmHandle->curSection->fullName)) {
			len = strlen(shortName) + strlen(parmHandle->curSection->fullName)
					+ 2;
			...
			snprintf(fullName, len, "%s/%s", parmHandle->curSection->fullName,
					shortName);
		} else {
		...
		if ((!parmHandle->curSection)
				|| (!strlen(parmHandle->curSection->fullName))) {
	...
	bailout: parmHandle->flag |= PARM_HANDLE_FLAG_PARSE_ERROR;
	return;
}
...
static int xmlExternalEntityRefHandler(XML_Parser mainparser,
	...
	parser = XML_ExternalEntityParserCreate(mainparser, openEntityNames,
			(const XML_Char *) NULL);
 ...
 *  <br>1 error
 */
static int parseXml(struct parmHandle *parmHandle, char *buf, int len,
		int done) {
...
static int parserXmlInit(struct parmHandle *parmHandle) {
	...
	XML_SetExternalEntityRefHandler(parmHandle->parser,
 ...
 *  <br>0 done
 */
static int xmlGetOuputLine(struct parmHandle *parmHandle, char *buffer,
		int size) {
	...
	while (1) {
		...
		case 4: /* Parse section attributes list */
			outCtrl->curParam = GF_TAILQ_FIRST(
					&(outCtrl->curSection->paramList));
			...
			if (curParam->type == P_STR) {
				...
				if (curParam->unit) {
					...
					if ((curParam->min != curParam->valnum)
						...
						snprintf(numvalue, NUMVALUE,
								" min=\"%g\" max=\"%g\" val=\"%g\"/>\n",
								curParam->min, curParam->max, curParam->valnum);
					} else {
				...
				createIndent(indent, INDENTSIZE, outCtrl->indent);
				snprintf(buffer, size, "%s<attnum name=\"%s\"%s", indent,
						curParam->name, numvalue);
...
static void evalUnit(char *unit, tdble *dest, int flg) {
	...
	if ((strcmp(unit, "feet") == 0) || (strcmp(unit, "ft") == 0)) {
		...
		coeff = 0.01f; /* m */
	} else if ((strcmp(unit, "in") == 0) || (strcmp(unit, "inch") == 0)
 ...
 *   		produce an incoherent pointer.
 */
const char* GfParmGetStr(void *parmHandle, const char *path, const char *key,
		const char *deflt) {
	/*
	 path = "Movie Capture"
	 key = "enable capture"
 ...
 *  @see GfParmListSeekNext
 */
const char* GfParmGetCurStr(void *handle, const char *path, const char *key,
		const char *deflt) {
	struct parmHandle *parmHandle = (struct parmHandle *) handle;
	struct parmHeader *conf = parmHandle->conf;
	struct section *section;
 ...
 *  @see GfParmListSeekNext
 */
tdble GfParmGetCurNum(void *handle, const char *path, const char *key,
		const char *unit, tdble deflt) {
	struct parmHandle *parmHandle = (struct parmHandle *) handle;
	struct parmHeader *conf = parmHandle->conf;
	struct section *section;
 ...
 *  <br>-1 error
 */
int GfParmSetStr(void *handle, const char *path, const char *key,
		const char *val) {
	struct parmHandle *parmHandle = (struct parmHandle *) handle;
	struct parmHeader *conf = parmHandle->conf;
	struct param *param;
 ...
 *  @see GfParmListSeekNext
 */
int GfParmSetCurStr(void *handle, const char *path, const char *key,
		const char *val) {
	struct parmHandle *parmHandle = (struct parmHandle *) handle;
	struct parmHeader *conf = parmHandle->conf;
	struct section *section;
	...
	param = getParamByName(conf, section->curSubSection->fullName, key,
			PARAM_CREATE);
	if (!param) {
		return -1;
	}
 ...
 *  <br>-1	error
 */
int GfParmSetNum(void *handle, const char *path, const char *key,
		const char *unit, tdble val) {
	struct parmHandle *parmHandle = (struct parmHandle *) handle;
	struct parmHeader *conf = parmHandle->conf;
	struct param *param;
 ...
 *   		<br>-1	error
 */
int GfParmSetNumEx(void *handle, const char *path, const char *key,
		const char *unit, tdble val, tdble min, tdble max) {
	struct parmHandle *parmHandle = (struct parmHandle *) handle;
	struct parmHeader *conf = parmHandle->conf;
	struct param *param;
 ...
 *  @see GfParmListSeekNext
 */
int GfParmSetCurNum(void *handle, const char *path, const char *key,
		const char *unit, tdble val) {
	struct parmHandle *parmHandle = (struct parmHandle *) handle;
	struct parmHeader *conf = parmHandle->conf;
	struct section *section;
...
int GfParmCheckHandle(void *ref, void *tgt) {
	...
	while (curSectionRef) {
		...
		while (curParamRef) {
			/* compare params */
			curParam = getParamByName(conf, curSectionRef->fullName,
					curParamRef->name, 0);
			if (curParam) {
				if (curParamRef->type != curParam->type) {
					...
					if ((curParam->valnum < curParamRef->min)
							|| (curParam->valnum > curParamRef->max)) {
						GfError(
								"GfParmCheckHandle: parameter \"%s\" out of bounds: min:%g max:%g val:%g in (\"%s\" - \"%s\")\n",
								curParamRef->fullName, curParamRef->min,
								curParamRef->max, curParam->valnum, conf->name,
								conf->filename);
					}
				} else {
					...
					while (!found && curWithinRef) {
						if (!strcmp(curWithinRef->val, curParam->value)) {
							...
							curWithinRef = GF_TAILQ_NEXT(curWithinRef,
									linkWithin);
						}
					}
					if (!found && strcmp(curParamRef->value, curParam->value)) {
						GfError(
								...
								curParamRef->fullName, curParam->value,
 ...
 * 
 */
static void insertParamMerge(struct parmHandle *parmHandle, char *path,
		struct param *paramRef, struct param *param) {
	struct parmHeader *conf = parmHandle->conf;
 ...
 * 
 */
static void insertParam(struct parmHandle *parmHandle, char *path,
		struct param *param) {
	struct parmHeader *conf = parmHandle->conf;
	struct param *paramNew;
	struct within *within;
...
void *GfParmMergeHandles(void *ref, void *tgt, int mode) {
	...
	GfOut("Merging \"%s\" and \"%s\" (%s - %s)\n", confRef->filename,
			confTgt->filename, ((mode & GFPARM_MMODE_SRC) ? "SRC" : ""),
			((mode & GFPARM_MMODE_DST) ? "DST" : ""));
	...
	if (mode & GFPARM_MMODE_SRC) {
		...
		while (curSectionRef) {
			...
			while (curParamRef) {
				/* compare params */
				curParamTgt = getParamByName(confTgt, curSectionRef->fullName,
						curParamRef->name, 0);
				if (curParamTgt) {
					insertParamMerge(parmHandleOut, curSectionRef->fullName,
							curParamRef, curParamTgt);
				} else {
					insertParam(parmHandleOut, curSectionRef->fullName,
							curParamRef);
				}
				curParamRef = GF_TAILQ_NEXT(curParamRef, linkParam);
			}
	...
	if (mode & GFPARM_MMODE_DST) {
		...
		while (curSectionTgt) {
			...
			while (curParamTgt) {
				/* compare params */
				curParamRef = getParamByName(confRef, curSectionTgt->fullName,
						curParamTgt->name, 0);
				if (curParamRef) {
					insertParamMerge(parmHandleOut, curSectionTgt->fullName,
							curParamRef, curParamTgt);
				} else {
					insertParam(parmHandleOut, curSectionTgt->fullName,
							curParamTgt);
				}
 ...
 */
int GfParmGetNumBoundaries(void *handle, const char *path, const char *key,
		tdble *min, tdble *max) {
	struct parmHandle *parmHandle = (struct parmHandle *) handle;
	struct parmHeader *conf = parmHandle->conf;
	struct param *param;
